...
void setup_llm_grammars()
{
  ...
  for (int iter = 0; iter < TEMPLATE_CONSISTENCY_COUNT; iter++)
  {
    ...
    char *templates_answer = chat_with_llm(templates_prompt, "glm-4.5-flash", GRAMMAR_RETRIES, 0.5);
    ...
    char *remaining_templates = chat_with_llm(remaining_prompt, "glm-4.5-flash", GRAMMAR_RETRIES, 0.5);
  ...
  if (uninteresting_times >= UNINTERESTING_THRESHOLD && chat_times < CHATTING_THRESHOLD)
  {
    ...
    if (responses_temp != NULL)
    {
      ...
      if (!empty)
      {
        ...
        // printf("Got prompt:\n\n%s\n",stall_prompt);
        char *stall_response = chat_with_llm(stall_prompt, "glm-4.5-flash", STALL_RETRIES, 1.5);
        // printf("Got response:\n\n%s\n",stall_response);
